...
$count_query = "SELECT COUNT(*) as total FROM tickets WHERE status = 'Pending'";
$count_result = $conn->query($count_query);
$total_tickets = $count_result->fetch_assoc()['total'];
$total_pages = max(1, ceil($total_tickets / $tickets_per_page));
// Fetch pending tickets
$query = "SELECT t.id, e.name, t.issue_description, t.issue_type, t.status, t.solution, t.created_at FROM tickets t LEFT JOIN employees e ON t.employee_id = e.id WHERE t.status = 'Pending' ORDER BY t.created_at DESC LIMIT $tickets_per_page OFFSET $offset";
...
<head>
    ...
    <link href="pending_tickets.css" rel="stylesheet">
</head>
...
<div class="header d-flex align-items-center" style="border-bottom:1px solid #eee;">
    ...
    <h1 class="mb-0">Pending Tickets</h1>
</div>
    ...
    <div class="table-responsive">
        <table class="table table-bordered table-hover align-middle">
            ...
            <tbody>
                ...
                <?php endwhile; else: ?>
                <tr><td colspan="7" class="text-center text-muted">No pending tickets found.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
        <!-- Pagination Controls -->
        <nav aria-label="Pending ticket pagination">
